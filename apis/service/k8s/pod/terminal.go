...
func handleTerminalSession(session sockjs.Session) {
	...
	if buf, err = session.Recv(); err != nil {
		log.Debug("handleTerminalSession: can't Recv: %v", err)
		return
	}

	if err = json.Unmarshal([]byte(buf), &msg); err != nil {
		log.Debug("handleTerminalSession: can't UnMarshal (%v): %s", err, buf)
		return
	}

	if msg.Op != "bind" {
		log.Debug("handleTerminalSession: expected 'bind' message, got: %s", buf)
		return
	}

	if terminalSession = terminalSessions.Get(msg.SessionID); terminalSession.id == "" {
		log.Debug("handleTerminalSession: can't find session '%s'", msg.SessionID)
		return
	}
...
// CreateAttachHandler is called from main for /api/v1/sockjs
...
func startProcess(namespace, podName, containerName string, cmd []string, ptyHandler PtyHandler) error {
	...
	if err != nil {
		log.Error("exec.Stream: ", err.Error())
		return err
	}
...
func WaitForTerminal(namespace, podName, containerName, shell, sessionId string) {
	...
	case <-terminalSessions.Get(sessionId).bound:
		...
		terminalSessions.Get(sessionId).Write([]byte("Process exited"))
